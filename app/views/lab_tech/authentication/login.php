...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    $sql = sprintf("SELECT * FROM labtechnician
                    WHERE email = '%s'",
                   $mysqli->real_escape_string($_POST["email_address"]));
    
    $result = $mysqli->query($sql);
    
    $labtechnician = $result->fetch_assoc();
    
    if ($labtechnician) {
        
        if (password_verify($_POST["password"], $labtechnician["password_hash"])) {
            
            session_start();
            
            session_regenerate_id();
            
            $_SESSION["labtechnician_id"] = $labtechnician["id"];
            
            header("Location: /lab tech/patient.html");
            exit;
        }
    }
