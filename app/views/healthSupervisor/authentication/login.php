...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    $sql = sprintf("SELECT * FROM healthsupervisor
                    WHERE email = '%s'",
                   $mysqli->real_escape_string($_POST["email_address"]));
    
    $result = $mysqli->query($sql);
    
    $healthsupervisor = $result->fetch_assoc();
    
    if ($healthsupervisor) {
        
        if (password_verify($_POST["password"], $healthsupervisor["password_hash"])) {
            
            session_start();
            
            session_regenerate_id();
            
            $_SESSION["healthsupervisor_id"] = $healthsupervisor["id"];
            
            header("Location: /healthSupervisor/healthSupervisor_dashboard.html");
            exit;
        }
    }
